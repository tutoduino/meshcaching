...
//
// Module purement protocolaire : aucune dependance materielle, aucune
// politique applicative (le choix de "qui est le repeteur cible" reste
// dans l'application).
// =====================================================================
namespace meshcore {
...
constexpr size_t kAdvertPubkeyLen = 32;  // une annonce embarque la cle publique complete
constexpr size_t kTracePingLen = 12;     // header + path_len + tag + auth + flags + 1 hash

// Vue decodee d'un paquet : les pointeurs designent le tampon d'origine,
// rien n'est copie.
struct PacketView {
  uint8_t routeType;
  uint8_t payloadType;
...
// Decoupe un paquet brut. Renvoie false si le paquet est incoherent.
bool parse(const uint8_t *raw, size_t len, PacketView &out);

// Identifiant du dernier noeud ayant emis ce paquet : dernier hash du
// chemin, ou l'emetteur lui-meme pour un paquet zero-hop. Renvoie false
// si indeterminable — c'est le cas des paquets TRACE, qui n'ont pas de
// hash en tete de payload (cf. traceTag()).
...
// trop court). C'est ce tag, renvoye tel quel par le noeud trace, qui
// permet d'apparier une reponse a notre ping.
bool traceTag(const PacketView &pkt, uint32_t &tag);

// Construit un ping TRACE zero-hop vers un seul noeud, identifie par le
// premier octet du hash de sa cle publique. out doit faire au moins
// kTracePingLen octets ; renvoie la taille ecrite.
// Format du payload (wiki MeshCore, "Companion Radio Protocol") :
//   [tag 4o][auth_code 4o][flags 1o][liste de hash a tracer]
